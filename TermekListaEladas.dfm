object FormTermekListaEladas: TFormTermekListaEladas
  Left = 309
  Top = 179
  BorderIcons = []
  BorderStyle = bsDialog
  Caption = 'Termék eladás'
  ClientHeight = 633
  ClientWidth = 602
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnActivate = FormActivate
  OnShortCut = FormShortCut
  PixelsPerInch = 96
  TextHeight = 20
  object GroupBox1: TGroupBox
    Left = 5
    Top = 5
    Width = 591
    Height = 451
    Caption = ' Termékek listája '
    TabOrder = 0
    object StringGridTermek: TStringGrid
      Left = 10
      Top = 50
      Width = 571
      Height = 391
      DefaultColWidth = 109
      DefaultRowHeight = 22
      FixedCols = 0
      RowCount = 2
      GridLineWidth = 0
      Options = [goRowSelect]
      ScrollBars = ssVertical
      TabOrder = 2
      OnClick = StringGridTermekClick
    end
    object EditSearchNev: TEdit
      Left = 10
      Top = 20
      Width = 223
      Height = 28
      TabOrder = 0
      OnChange = EditSearchNevChange
    end
    object EditSearchVonalkod: TEdit
      Left = 265
      Top = 20
      Width = 96
      Height = 28
      TabOrder = 1
      OnChange = EditSearchVonalkodChange
    end
    object BitBtnSortCode: TBitBtn
      Left = 360
      Top = 20
      Width = 31
      Height = 30
      Default = True
      TabOrder = 3
      OnClick = BitBtnSortCodeClick
      Glyph.Data = {
        
        33333333337F7F33333333333300033333333333337773333333}
      Margin = 5
      NumGlyphs = 2
    end
    object BitBtnSortName: TBitBtn
      Left = 233
      Top = 20
      Width = 31
      Height = 30
      Default = True
      Enabled = False
      TabOrder = 4
      OnClick = BitBtnSortNameClick
      Glyph.Data = {
        
        33333333337F7F33333333333300033333333333337773333333}
      Margin = 5
      NumGlyphs = 2
    end
  end
  object BitBtnOk: TBitBtn
    Left = 119
    Top = 595
    Width = 150
    Height = 30
    Caption = 'Fizetés azonnal'
    Default = True
    ModalResult = 1
    TabOrder = 3
    Glyph.Data = {
      
      D4D4C6C6C6C3C3C3E5E5E5FF00FFFF00FFFF00FFFF00FFFF00FF}
    Margin = 5
    NumGlyphs = 2
  end
  object BitBtnKosar: TBitBtn
    Left = 276
    Top = 595
    Width = 100
    Height = 30
    Caption = 'Kosárba'
    Default = True
    ModalResult = 6
    TabOrder = 4
    Glyph.Data = {
      
      888DDDDDD8887877888DDDDDDDD88888888DDDDDDDD88888888D}
    Margin = 5
    NumGlyphs = 2
  end
  object BitBtnCancel: TBitBtn
    Left = 384
    Top = 595
    Width = 100
    Height = 30
    Caption = 'Mégsem'
    TabOrder = 5
    Kind = bkCancel
    Margin = 5
  end
  object GroupBox2: TGroupBox
    Left = 5
    Top = 520
    Width = 591
    Height = 66
    Caption = ' Termék vásárlás '
    TabOrder = 2
    object Label5: TLabel
      Left = 5
      Top = 30
      Width = 162
      Height = 20
      Caption = 'Ennyi darab eladása : '
    end
    object Label6: TLabel
      Left = 215
      Top = 30
      Width = 82
      Height = 20
      Caption = 'Fizetendõ :'
    end
    object ComboBoxDarab: TComboBox
      Left = 170
      Top = 27
      Width = 41
      Height = 28
      Style = csDropDownList
      ItemHeight = 20
      TabOrder = 0
      OnChange = ComboBoxDarabChange
    end
    object EditFizetendo: TEdit
      Left = 300
      Top = 27
      Width = 56
      Height = 28
      Enabled = False
      TabOrder = 1
    end
  end
  object GroupBox3: TGroupBox
    Left = 5
    Top = 455
    Width = 591
    Height = 66
    Caption = ' Eladandó termék adatai '
    TabOrder = 1
    object Label1: TLabel
      Left = 10
      Top = 30
      Width = 104
      Height = 20
      Caption = 'Termék neve: '
    end
    object EditNev: TEdit
      Left = 115
      Top = 27
      Width = 256
      Height = 28
      Enabled = False
      TabOrder = 0
    end
    object EditVonalkod: TEdit
      Left = 0
      Top = 27
      Width = 26
      Height = 28
      Enabled = False
      TabOrder = 1
      Visible = False
    end
    object EditAr: TEdit
      Left = 25
      Top = 27
      Width = 26
      Height = 28
      Enabled = False
      TabOrder = 2
      Visible = False
    end
    object EditDarab: TEdit
      Left = 50
      Top = 27
      Width = 21
      Height = 28
      Enabled = False
      TabOrder = 3
      Visible = False
    end
    object EditID: TEdit
      Left = 0
      Top = 10
      Width = 21
      Height = 28
      Enabled = False
      TabOrder = 4
      Visible = False
    end
  end
end
